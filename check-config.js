...
// Тесты режут файл по ПЕРВОМУ вхождению этих четырёх строк, поэтому каждая должна
// встречаться ровно один раз. Один раз оглавление в начале файла уже упомянуло
// «const rub =» и «ЯДРО: конец.» дословно — и оба теста стали резать комментарий
// вместо кода. Считаем вхождения, чтобы это не повторилось молча.
[["const rub =", "test.js"],
 ["/* ----------------------------- Утилиты", "test.js"],
 ["const CP1251_HIGH =", "test-headcount.js"],
 ["ЯДРО: конец.", "test-headcount.js"]].forEach(([m, who]) => {
  const n = html.split(m).length - 1;
  if (n > 1) {
    problems.push("строка «" + m + "» встречается в index.html " + n + " раза, а должна один: "
      + who + " режет файл по первому вхождению и возьмёт не тот кусок");
  }
});
