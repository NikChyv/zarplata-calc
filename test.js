...
// ВАЖНО: взнос в ФСЗН (1%) налоговую базу НЕ уменьшает — его удерживают из выплаты,
// но подоходный налог считается без его вычета. Это решение заказчика, оно отличается
// от исходного ТЗ. Базу уменьшают только ДПНС и вычеты.

console.log("--- 1. Прямой расчёт, зарплата выше порога, без опций (Gross 2000) ---");
let r = calc(200000, P());
eq("ФСЗН 1% удержан", f(r.fsznK), "20.00");
eq("но базу не уменьшил: облагаемый доход = начислено", f(r.subjectK), "2000.00");
eq("личный вычет не применяется", r.personalK, 0);
eq("налог 13% от 2000", f(r.taxK), "260.00");
eq("на руки 2000-20-260", f(r.netK), "1720.00");
...
eq("облагаемый доход 2000-60 (ФСЗН не вычитается)", f(r.subjectK), "1940.00");
eq("вычет на ребёнка", f(r.childrenK), "63.00");
eq("база 1940-63-100", f(r.baseK), "1777.00");
eq("налог", f(r.taxK), "231.01");
eq("профсоюз 1%", f(r.unionK), "20.00");
eq("на руки 2000-20-60-231.01-20", f(r.netK), "1668.99");

console.log("\n--- 3. Порог личного вычета (216 р. при облагаемом доходе <= 1308) ---");
// Без ДПНС облагаемый доход равен начисленному, поэтому порог достигается ровно на 1308.00
r = calc(130800, P());
...
console.log("\n--- 4. Разрыв функции на пороге: на руки падает примерно на 28 р. ---");
const netLo = calc(130800, P()).netK, netHi = calc(130801, P()).netK;
...
{
  const p = P();
  const target = calc(130800, p).netK;              // достижимо на нижней ветке
  const g = grossFromNet(target, p);
  eq("выбран Gross нижней ветки", f(g.grossK), f(130800));
...
console.log("\n--- 9. Договор подряда (ГПХ): вычеты не предоставляются ---");
{
  ...
  eq("по подряду база = весь облагаемый доход", f(gph.baseK), f(gph.subjectK));
  eq("налог по подряду 13% от 1200", f(gph.taxK), "156.00");
  eq("на руки по подряду 1200-12-156", f(gph.netK), "1032.00");
